package com.nd.web.iphone.view.component
{
	...
	public class PictureCanvas extends Sprite implements IEventDispatcher
	{
		 ...
		 * 设置水印图片
		 */
		public function SetWater(waterImage:BitmapData):void
		{
			this.cWater.x=this.cSelector.x;
			this.cWater.y=this.cSelector.y;
			this.cWater.Show(waterImage);
			//this.cWater.addEventListener(MouseEvent.MOUSE_DOWN, onWaterMouse_Down);
			//this.cWater.addEventListener(MouseEvent.MOUSE_UP, onWaterMouse_Up);
			//this.cWater.addEventListener(MouseEvent.MOUSE_MOVE, onWaterMouse_Move);
			this.cWater.visible=true;
		}

		/**
		 * 隐藏未应用的水印
		 */
		public function HideWater():void
		{
			this.cWater.visible=false;
		}

		/**
		 * 应用水印图片
		 */
		public function ApplyWater():void
		{
			var matrix:Matrix=new Matrix();
			matrix.scale(1 / this.cImage.Scale, 1 / this.cImage.Scale);
			matrix.translate((this.cWater.RealX - this.cImage.x) / this.cImage.Scale, (this.cWater.RealY - this.cImage.y) / this.cImage.Scale);
			this.cImage.LoadPicture(BitmapDataHelper.WaterMark(this.cImage.ImageSource, this.cWater.ImageSource, matrix));
		}
		 ...
		 * @private
		 */
		private function onWaterMouse_Down(e:MouseEvent):void
		{
			e.stopPropagation();
			this.cWater.startDrag(false);
		}

		/**
		 * @private
		 */
		private function onWaterMouse_Move(e:MouseEvent):void
		{
			e.stopPropagation();
		}
		 ...
		 * @private
		 */
		private function onWaterMouse_Up(e:MouseEvent):void
		{
			e.stopPropagation();
			this.cWater.stopDrag();
		}
