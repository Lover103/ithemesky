<%@ Control Language="C#" Inherits="System.Web.Mvc.ViewUserControl<SoftCommentListModel>" %>
<h3 class="commentHead"><%=ViewData.Model.RecordCount %> Comments:</h3>
<ul class="commentList">
    <% 
        int index = 1;
        foreach (SoftComment comment in ViewData.Model.Comments)
       { %>
...
<div class="commentPage clearfix">
	<% Html.RenderPagination("javascript:LoadSoftComments({0})", ViewData.Model.PageIndex, ViewData.Model.PageSize, ViewData.Model.RecordCount, 10, false, false); %>
</div>