<%@ Control Language="C#" Inherits="System.Web.Mvc.ViewUserControl<PostSoftCommentModel>" %>
<iframe id="hiddenIframe" name="hiddenIframe" src="#" style="display:none"></iframe>
<% using (Html.BeginForm("AddSoftComment", "Service", FormMethod.Post, new { target = "hiddenIframe" }))
   { %>
    <%= Html.HiddenFor(m=> m.SoftIdentify) %>
    <%= Html.HiddenFor(m=> m.SoftTitle) %>
    <%= Html.HiddenFor(m=> m.SoftDescription) %>
